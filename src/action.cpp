...
void AskAction::TakeEffect(CVC& gamestate) {
    ...
    if (success) {
        ...
        this->GetActor()->SetMoney(this->GetActor()->GetMoney() - this->request_amount_);
        this->target_->SetMoney(this->target_->GetMoney() + this->request_amount_);

        //increase opinion of actor (got money)
        this->GetActor()->AddRelationship(std::make_unique<RelationshipModifier>(this->target_, gamestate.Now(), gamestate.Now() + 10, this->request_amount_));
        //decrease opinion of target (gave money)
        this->target_->AddRelationship(std::make_unique<RelationshipModifier>(this->GetActor(), gamestate.Now(), gamestate.Now() + 10, -1.0 * this->request_amount_));

        printf("request by %d to %d of %f\n", this->GetActor()->GetId(), this->target_->GetId(), this->request_amount_);
    } else {
        //on failure:
