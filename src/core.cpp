...
CVC::CVC(std::unique_ptr<DecisionEngine> decision_engine,
         std::vector<std::unique_ptr<Character>> characters,
         std::mt19937 random_generator, FILE* action_log) {
  this->ticks_ = 0;
...
int CVC::Now() const { return this->ticks_; }
  ...
  for (auto& character : this->characters_) {
    ...
    while (true) {
      ...
      std::vector<std::unique_ptr<Action>> actions =
          this->decision_engine_->EnumerateActions(*this, character.get());
